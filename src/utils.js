...
{
    const output = objs.map(function(x){return(<>{x}</>);});
    return (<>{output}</>);
}
...
export function getRawNumberAndSuffix(arg)
{
    ...
    iend++;
    const val = parseFloat(arg.substring(0, iend));
    const suffix = arg.substring(iend, arg.length);
    return {suffix: suffix, val: val};
}
